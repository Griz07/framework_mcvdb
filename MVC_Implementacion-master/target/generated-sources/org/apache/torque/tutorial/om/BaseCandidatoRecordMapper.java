 ...
 *
 */
public class BaseCandidatoRecordMapper implements RecordMapper<Candidato>
{

    /** The class log. */
    private static Log log
            = LogFactory.getLog(BaseCandidatoRecordMapper.class);
     ...
     *         than the columns in the candidato table.
     */
    public Candidato processRow(
                ResultSet resultSet,
                int offset,
            ...
            throws TorqueException
    {
        ...
        try 
        {
            candidato.setLoading(true);
            if (criteria == null)
            {
                candidato.setCandidatoId(
                        getCandidatoId(resultSet, offset + 1));
                candidato.setNombre(
                        getNombre(resultSet, offset + 2));
                candidato.setNumVotos(
                        getNumVotos(resultSet, offset + 3));
            }
            else
            {
                ...
                {
                    if (BaseCandidatoPeer.CANDIDATO_ID.getSqlExpression().equals(
                            column.getSqlExpression()))
                    {
                        candidato.setCandidatoId(
                            getCandidatoId(resultSet, totalOffset));
                        columnMapped = true;
                    }
                    else if (BaseCandidatoPeer.NOMBRE.getSqlExpression().equals(
                            column.getSqlExpression()))
                    {
                        candidato.setNombre(
                    ...
                    else if (BaseCandidatoPeer.NUM_VOTOS.getSqlExpression().equals(
                            column.getSqlExpression()))
                    {
                        candidato.setNumVotos(
                            getNumVotos(resultSet, totalOffset));
                        columnMapped = true;
                    }
                    totalOffset++;
                }
            ...
            candidato.setModified(false);
        }
        finally
        {
            candidato.setLoading(false);
        }
        return candidato;
    }
         ...
         * the field candidatoId in Candidato.
         *
         ...
         * Reads the value of the <code>columnIndex</code>'th column
         * in the <code>resultSet</code> so that it can be used to set
         * the field numVotos in Candidato.
         *
         * @param dbObject the database object to fill, not null.
         * @param resultSet the ResultSet to read from, not null.
         * @param columnIndex the index in the resultSet which should be read.
